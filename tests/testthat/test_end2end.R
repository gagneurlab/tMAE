test_that("End to end", {
    ...
    if(requireNamespace(pkg_name, quietly=TRUE)){
        res <- add_gnomAD_AF(data=maeRes, genome_assembly='hg38', pops="AF")
        res <- add_gnomAD_AF(data=maeRes, genome_assembly='GRCh38', pops="AF")
    }
    
    # expect hg19 to work or to download it on the fly 
    # only use the exome version to save space for testing
    res <- add_gnomAD_AF(data=maeRes, genome_assembly='MafDb.ExAC.r1.0.hs37d5', pops="AF")
